# NAME: Mohamed Elshabrawy
# STUDENT NUMBER: 250973600
# PARTNERS: Arrian Gibson
...
def make_average_change_dict_for_map(station_names, station_name_dict, station_data, temperature_data_dict, start_year, end_year):
    ...
    Here again you are asked to create and fill a dictionary. This time, it needs to have the right keys and values
    so that it can be passed to the draw_map function. As is indicated in the return comment below, this dictionary
    needs to map station names (strings) to a tuple that includes the latitude, longitude, and average temperature change.

    HINT: The average temperature changes can be generated by calling the previous function, which returns a dictionary.
